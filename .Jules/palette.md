...
## 2026-02-03 - Stable Toggle Flex Layouts
**Learning:** When using text characters (like ✓) conditionally in flexbox toggle buttons, omitting them can cause layout shift, and displaying them can cause screen readers to announce them redundantly.
**Action:** Always wrap decorative toggle characters in an `aria-hidden='true'` span with a fixed dimension, and provide an empty placeholder of the same size when the condition is false.
