...
## 2026-02-02 - Toggle Button Layout Shift and Accessibility
**Learning:** Conditional decorative characters like '✓' in flex buttons cause layout shift (jitter) when toggled and are read aloud by screen readers. Placing them in a fixed-size 'aria-hidden' container solves both issues.
**Action:** Always wrap text-based decorative icons in a span with 'aria-hidden="true"' and fixed dimensions, providing an empty placeholder of the same size when inactive to prevent layout shift.
