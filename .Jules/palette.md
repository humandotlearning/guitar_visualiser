...
## 2026-02-02 - Async Clipboard Feedback
**Learning:** Copy-to-clipboard actions are invisible to users without explicit feedback. A temporary icon swap (e.g., Copy -> Check) paired with `aria-label` updates provides immediate confirmation for both sighted and screen reader users.
**Action:** Always implement a temporary success state (icon change + `aria-label="Copied"`) for copy buttons, reverting after ~2 seconds.
