...
## 2026-06-04 - Decorative Icons and Layout Shift
**Learning:** Decorative characters (like ✓) in toggle buttons can cause layout shift when appearing/disappearing, and screen readers redundantly announce them. Additionally, in flex containers with a gap, mixing text nodes and elements can lead to alignment issues.
**Action:** Wrap decorative characters in a fixed-width `span` with `aria-hidden="true"` to prevent layout shift and screen reader clutter. Wrap the accompanying text in another `span` to ensure consistent flex `gap` alignment.
