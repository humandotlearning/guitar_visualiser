...
## 2026-02-02 - Hiding Text-Based Icons
**Learning:** Text characters used as icons (like ✓ or ✕) inside buttons are read aloud by screen readers (e.g., "Check Show Scale Notes"), which can clutter the auditory experience or confuse users, especially if the button already uses `aria-pressed`.
**Action:** Always wrap text-based decorative icons in a `<span aria-hidden="true">` to keep the auditory experience clean and semantically pure.
