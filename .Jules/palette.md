...
## 2026-04-26 - Hiding Decorative Characters from Screen Readers
**Learning:** Text characters used as decorative icons (like the checkmark ✓ in toggle buttons) are announced by screen readers, cluttering the auditory experience. Since the button state is already conveyed via `aria-pressed`, the text checkmark is redundant and confusing auditory noise.
**Action:** Always wrap decorative text characters (like ✓ or ✕) inside interactive elements with a `<span aria-hidden="true">` to prevent screen readers from redundantly announcing them.
